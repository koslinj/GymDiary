...
export const AddRemoveSets: FC<Props> = ({ length, routeKey }) => {
  ...
  const { addSet, removeSet } = useSets();

  return (
    length > 1 ? (
      <ThemedView className='flex-row justify-between'>
        <TouchableOpacity onPress={() => addSet(routeKey)} className='flex-row items-center space-x-2'>
        ...
        <TouchableOpacity onPress={() => removeSet(routeKey)} className='flex-row items-center space-x-2'>
